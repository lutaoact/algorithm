...
var LogSystem = function() {
  this.logs = [];
  this.unitMap = {Year: 4, Month: 7, Day: 10, Hour: 13, Minute: 16, Second: 19};
};
...
LogSystem.prototype.retrieve = function(s, e, gra) {
  let results = [], length = this.unitMap[gra];
  let sBase = s.substr(0, length), eBase = e.substr(0, length);
  for (let [id, timestamp] of this.logs) {
    if (timestamp.substr(0, length) >= sBase && timestamp.substr(0, length) <= eBase) {
      results.push(id);
    }
  }
  return results;
